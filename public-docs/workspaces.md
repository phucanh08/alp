---
title: Workspaces
description: Understand how alp groups working directories, agents, terminals, and browsers into workspaces.
nav: Workspaces
order: 10
category: Workspaces
---
...
alp is organized around workspaces, not chats.
...
The sidebar starts with projects. A project can be a git repository, a GitHub project, or any directory on a machine running the alp daemon.
...
In alp, the workspace is the stable container. The sessions are what you run inside it.
...
The workspace is the product concept; a git worktree is one way to isolate its files. More than one workspace can refer to the same managed worktree, and alp removes that worktree after its last workspace is archived.
...
Every workspace in alp is backed by a working directory. When that directory is a git worktree, you get a separate branch and isolated environment for each task.
