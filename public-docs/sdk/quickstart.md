...
`cwd` is the directory the agent works in. alp creates the workspace behind it. [Workspaces](/docs/sdk/workspaces) covers reusing one instead.
...
The agent keeps running on the daemon and stays visible in alp. A later program reaches it again by ID:
