...
Run `alp reload` after editing the file. Provider changes apply to future launches without restarting the daemon.
...
The examples below are a quick tour. The full, up-to-date reference is on GitHub: [docs/custom-providers.md](https://github.com/phucanh08/alp/blob/main/docs/custom-providers.md).
...
Any agent that speaks [ACP](https://agentclientprotocol.com) over stdio can be added with `extends: "acp"` and a `command`. alp spawns the process, sends an `initialize` JSON-RPC request, and the agent reports its capabilities, modes, and models at runtime.
