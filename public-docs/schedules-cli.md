...
description: Create and manage alp schedules with alp schedule.
nav: CLI
order: 27
category: Schedules
---
...
`alp schedule` creates and manages new-agent [schedules](/docs/schedules) from your terminal, useful for headless boxes and scripts. Every run starts a fresh agent.
...
Inside a running alp agent, create a heartbeat for that same conversation:

```bash
alp heartbeat create \
  --cron "*/20 * * * *" \
  --name heartbeat \
...
```bash
alp heartbeat update <id> --cron "*/10 * * * *"
...
```bash
alp schedule ls
alp schedule inspect <id>
alp schedule logs <id>
alp schedule pause <id>
alp schedule resume <id>
alp schedule run-once <id>
alp schedule update <id> --every 10m --max-runs 6
alp schedule delete <id>
```
...
Start the command with `alp --host <target> schedule create ...` when targeting a remote daemon. Pass `--cwd`; your local working directory may not exist on the remote machine.
