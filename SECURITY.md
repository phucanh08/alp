...
2. The pairing URL (rendered as a QR code or opened directly) carries the daemon's public key in its URL fragment (`https://app-alp.anhlp.com/#offer=...`). Fragments are not sent to the web server, so `app-alp.anhlp.com` never sees the key.
