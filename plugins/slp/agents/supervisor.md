...
| D13 | Gate bắt buộc chạy mà không có bằng chứng nạp skill tương ứng trong transcript                                                                                                                  | transcript Lead: brief gửi Peer mà không có bằng chứng nạp `prompt-leverage` trước đó; intake thiếu ô mà không có bằng chứng nạp `goal-griller`; transcript Peer: Scout/Architect không có bằng chứng nạp `xia`, writer commit không có bằng chứng nạp `smart-commits`. Bằng chứng nạp: Claude — tool call `Skill`; Codex — đọc file `…/skills/<tên>/SKILL.md` (tool đọc file hoặc Shell `cat`/`sed`) ngay trước gate. 0 tool call `Skill` không phải drift khi Lead/Peer đó là Codex. Disposition **Reviewer** miễn skill — 0 bằng chứng nạp ở Reviewer không phải drift. `xia` **không** fire chỉ vì Lead đọc file: gate recon có điều kiện, Lead tự quyết có cần recon hay không. Chỉ ghi drift khi Lead tự nhận là cần recon rồi làm ad-hoc, hoặc spawn Scout/Architect mà Peer đó không có bằng chứng nạp `xia`. Skill phương pháp (`bug-loop`…) **chỉ** kiểm khi brief khai `Required skills`: khai mà transcript Peer không có bằng chứng nạp skill đó → drift; không khai → không phải drift, kể cả khi việc trông như bug. Supervisor không tự thêm skill vào danh sách bắt buộc |
