...
alp passes those variables through to the Codex app-server process **and** maps them into Codex's thread config under `model_provider` / `model_providers`, because Codex reads provider routing from config rather than from `OPENAI_BASE_URL` alone.
...
Under the hood, for each custom Codex provider alp injects this into Codex's config:
...
- `base_url` — taken from `OPENAI_BASE_URL`. If it does not already end in `/v1`, alp appends `/v1`. Trailing slashes are stripped.
- `wire_api` — always `"responses"` (OpenAI Responses API protocol).
- `env_key` — set to `"OPENAI_API_KEY"` when that env var is present and non-empty, so Codex reads the key from the same env var alp passes through.
...
- Set `models` explicitly. Custom endpoints expose their own model IDs (`anthropic/claude-opus-4-7`, `qwen/qwen3-coder`, `local/llama`, etc.), and alp does not discover them automatically for Codex.
...
Each profile appears as a separate provider in the alp app. You can select which one to use when launching an agent.
...
The `command` array completely replaces the default command for that provider. The binary must exist on the system — alp checks for its availability and will mark the provider as unavailable if not found.
...
Custom OMP profiles should extend `omp`. They inherit the OMP adapter's `rpc-ui` approvals, native alp host tools, provider-managed subagents, and import behavior:
...
`params.sessionDir` is used only for importing sessions that were started outside alp. If `command` or XDG env vars move OMP's state directory, set `params.sessionDir` to the resulting OMP JSONL session directory; launching and resuming still go through the configured command. OMP waits 20 seconds for its initial `ready` frame and 60 seconds for later control-plane RPCs by default. `params.rpcTimeoutMs` overrides both deadlines.
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) is an open standard for communication between editors and AI coding agents — think LSP but for AI agents. Any agent that supports ACP can be added to alp as a custom provider.

ACP agents communicate over JSON-RPC 2.0 on stdio. alp spawns the agent process and talks to it through stdin/stdout.

alp also ships an in-app ACP provider catalog for common agents, including CodeWhale, Cursor, DeepAgents, DimCode, Gemini CLI, Hermes, Qwen Code, and Kimi Code. Catalog entries create the same `extends: "acp"` provider config shown below.
...
alp tools such as subagent creation come from the shared internal tool catalog. ACP providers receive those tools through the MCP fallback by default because ACP exposes `mcpServers`, not alp's native tool catalog. Some ACP adapters cannot create sessions when `mcpServers` is non-empty. Disable injected MCP for those providers with `params.supportsMcpServers: false`:
...
while terminal operations run through alp on the host. To customize which
operations alp handles, configure client capabilities in provider params:
...
container. When delegating filesystem operations to alp (`fs.readTextFile: true`
or `fs.writeTextFile: true`), ensure the agent and alp share equivalent
absolute workspace paths.

### Generic ACP diagnostics

alp diagnostics for `extends: "acp"` providers report the configured command, resolved launcher binary, version output, ACP `initialize`, ACP `session/new`, model count, modes, and final status.
