...
alp surfaces terminal activity as a tab indicator (the same "running" dot used by agents).
...
The plugin translates both event contracts into the existing alp hook events. OpenCode 2 disposes its event subscription when the plugin unloads.
...
When enabled, alp installs provider hooks globally:
