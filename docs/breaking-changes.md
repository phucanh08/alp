...
| 2026-09-25 | agent providers  | `agents.providers` has no SLP entries                                                 | daemon start seeds `claude-lead`, `claude-peer`, `claude-supervisor` (extending `claude`) and `codex-lead`, `codex-peer`, `codex-supervisor` (extending `codex`) into `agents.providers`, each only when its key is missing                             | six extra provider choices appear on new hosts; deleting one is undone on the next daemon start unless set to `enabled: false`; `codex-peer` loses its own agent spawning (`features.multi_agent: false`) only through the slp plugin, so with plugins disabled it can still spawn agents |
