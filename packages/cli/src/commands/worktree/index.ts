...
export function createWorktreeCommand(): Command {
  const worktree = new Command("worktree").description("Manage alp-managed git worktrees");

  addJsonAndDaemonHostOptions(
    worktree.command("ls").description("List alp-managed git worktrees"),
  ).action(withOutput(runLsCommand));

  addJsonAndDaemonHostOptions(
    worktree
      .command("create")
      .description("Create an alp-managed git worktree")
