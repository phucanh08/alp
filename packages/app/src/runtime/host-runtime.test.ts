...
function makeOffer(input?: Partial<ConnectionOffer>): ConnectionOffer {
  return {
    ...
    relay: {
      endpoint: input?.relay?.endpoint ?? "relay-alp.anhlp.com:443",
...
function encodeOfferUrl(payload: unknown): string {
  const encoded = Buffer.from(JSON.stringify(payload), "utf8")
    ...
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
  return `https://app-alp.anhlp.com/#offer=${encoded}`;
}
...
describe("HostRuntimeController", () => {
  ...
  it("replaces the active relay client when re-pairing changes the daemon public key", async () => {
    const oldRelay: HostConnection = {
      id: "relay:wss:relay-alp.anhlp.com:443",
      type: "relay",
      relayEndpoint: "relay-alp.anhlp.com:443",
      useTls: true,
  ...
  it("adopts the first successful probe on startup", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 82,
      "relay:relay-alp.anhlp.com:443": 18,
    };
    const controller = new HostRuntimeController({
      host,
    ...
    const controller = new HostRuntimeController({
      ...
      deps: {
        ...
        connectToDaemon: async ({ host: hostProfile, connection }) => {
          ...
          if (connection.id === "relay:relay-alp.anhlp.com:443") {
  ...
  it("ranks the live connection by its heartbeat RTT without pinging it again", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 12,
      "relay:relay-alp.anhlp.com:443": 65,
    };
    const controller = new HostRuntimeController({
      host,
      deps: {
  ...
  it("keeps the live connection when one probe cycle looks slow", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay-alp.anhlp.com:443": 55,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay-alp.anhlp.com:443"] = 42;
  ...
  it("backs off inactive connection probes while a host is online", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 10,
      "relay:relay-alp.anhlp.com:443": 50,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    const initialRelayProbe = controller
      .getSnapshot()
      .probeByConnectionId.get("relay:relay-alp.anhlp.com:443");

    latencies["direct:lan:6767"] = 12;
    latencies["relay:relay-alp.anhlp.com:443"] = 25;
    ...
    expect(snapshot.probeByConnectionId.get("relay:relay-alp.anhlp.com:443")).toEqual(
      initialRelayProbe,
    );
  });
  ...
  it("switches only after the faster alternative wins consecutive probes", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay-alp.anhlp.com:443": 60,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay-alp.anhlp.com:443"] = 30;
    ...
    let switched = controller.getSnapshot().activeConnectionId === "relay:relay-alp.anhlp.com:443";
    for (let index = 0; index < 6 && !switched; index += 1) {
      await vi.advanceTimersByTimeAsync(120_000);
      await controller.runProbeCycleNow();
      switched = controller.getSnapshot().activeConnectionId === "relay:relay-alp.anhlp.com:443";
    }
  ...
  it("does not switch on a transient latency spike", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay-alp.anhlp.com:443": 80,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay-alp.anhlp.com:443"] = 20;
    ...
    latencies["relay:relay-alp.anhlp.com:443"] = 90;
  ...
  it("ignores stale switch failures after a newer connection is already online", async () => {
    ...
    const switchRelay = controller.activateConnection({
      connectionId: "relay:relay-alp.anhlp.com:443",
    });
    await waitUntil(() => {
      const snapshot = controller.getSnapshot();
      return (
        snapshot.activeConnectionId === "relay:relay-alp.anhlp.com:443" &&
        snapshot.connectionStatus === "online"
      );
    });
    ...
    const snapshot = controller.getSnapshot();
    expect(snapshot.activeConnectionId).toBe("relay:relay-alp.anhlp.com:443");
    expect(snapshot.connectionStatus).toBe("online");
  ...
  it.each(["active", "inactive", "background"] as const)(
    ...
    async (currentState) => {
      const relay = (suffix: string): HostConnection => ({
        id: `relay:relay-${suffix}.alp.anhlp.com:443`,
        type: "relay",
        relayEndpoint: `relay-${suffix}.alp.anhlp.com:443`,
