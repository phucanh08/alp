...
async function main() {
  ...
  try {
    ...
    const helper = fs.statSync("/opt/alp/chrome-sandbox");
    ...
    await smokePackagedDesktopApp({ appPath: "/opt/alp", expectedSandbox: true });
    ...
    const desktopEntry = fs.readFileSync(path.join(appDir, "alp.desktop"), "utf8");
    ...
    const appPath = fs.existsSync(path.join(tarDir, "alp"))
      ? tarDir
