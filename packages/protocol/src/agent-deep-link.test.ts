...
describe("agent deep links", () => {
  it("round-trips an existing agent target", () => {
    ...
    expect(link).toBe("alp://h/server%2Fmain/agent/agent%20123");
  ...
  it("rejects links outside the exact agent route", () => {
    ...
    expect(parseAgentDeepLink("alp://app/h/server/agent/agent-1")).toBeNull();
    expect(parseAgentDeepLink("alp://h/server/agent/agent-1?message=hello")).toBeNull();
    expect(parseAgentDeepLink("alp://h/server/agent/agent-1/extra")).toBeNull();
  });
});
