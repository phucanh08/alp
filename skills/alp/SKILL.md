---
name: alp
description: alp reference for managing projects, workspaces, workspace scripts, agents, schedules, and heartbeats.
---

alp is a remote daemon that manages coding agents, terminals. Control it through MCP tools or the CLI.
...
```bash
alp project create [path]
alp project ls
alp project rename <project-id> <name>
...
Choose `baseBranch` explicitly: `origin/main` selects the remote-tracking branch; `refs/heads/main` selects local main. Bare `main` prefers local main when it exists, otherwise origin/main. alp retains the resolved ref for workspace comparisons, even after rebasing the branch or changing its PR target.

**`list_workspaces`** — list active workspaces.

**`archive_workspace`** — `{ workspaceId }`. Archives the workspace, its agents, and its terminals. Local directories remain; alp removes an owned worktree only after its final active workspace reference is archived.
...
**`start_workspace_script`** — `{ workspaceId, scriptName }`. Starts one configured script through alp's managed workspace-script launcher and returns its status metadata.
...
```bash
alp workspace create --isolation worktree --mode branch-off --new-branch fix-x --base origin/main
alp workspace create --isolation worktree --mode checkout-branch --branch existing-work
alp workspace create --isolation worktree --mode checkout-pr --pr-number 42
alp run --provider codex/gpt-5.4 --mode full-access --workspace <workspace-id> "<prompt>"
alp run --provider codex/gpt-5.4 --mode full-access --new-workspace worktree --worktree-mode branch-off --new-branch fix-x --base origin/main "<prompt>"
alp send <agent-id> "<follow-up>"
alp ls
alp schedule create --cron "*/15 * * * *" "ping main build"
alp heartbeat create --cron "*/15 * * * *" "check the build"
```

Discover with `alp --help` and `alp <cmd> --help`.

For product questions, setup, logs, version problems, or troubleshooting, use the **alp-help** skill.
