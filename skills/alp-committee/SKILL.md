...
Read the **alp** skill. Call `list_profiles` before choosing committee members. Do not create committee agents until you have read the configured profiles and their `notes`.
...
If the user names profiles, use those. If fewer than two suitable profiles are configured, use alp's provider discovery fallback for the missing member and tell the user. Override the selection only when the user explicitly asks for different members.
...
- **Trust the finish notification.** Do not poll, send hurry-ups, or interrupt. Models can reason for 15–30 minutes. You can go idle and alp will notify you.

## Workflow

1. Write a problem-level prompt
2. Create both agents in parallel via alp with `[Committee] <task>` titles and the same prompt
